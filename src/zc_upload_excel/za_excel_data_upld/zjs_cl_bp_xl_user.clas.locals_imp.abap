CLASS lhc_XLHead DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
    ...
    METHODS autoReadExcel FOR DETERMINE ON MODIFY
      IMPORTING keys FOR XLHead~autoReadExcel.
...
CLASS lhc_XLHead IMPLEMENTATION.
  ...
  METHOD uploadExcelData.
    ...
    DATA: lt_rows         TYPE STANDARD TABLE OF string,
          ...
          lt_excel        TYPE STANDARD TABLE OF zjs_cl_bp_xl_user=>gty_gr_xl,
          lt_data         TYPE TABLE FOR CREATE zcjs_i_xl_user\_XLData,
    ...
    READ ENTITIES OF zcjs_i_xl_user IN LOCAL MODE
    ENTITY XLHead
    ...
    IF lt_existing_XLData IS NOT INITIAL.
      MODIFY ENTITIES OF zcjs_i_xl_user IN LOCAL MODE
  ...
  METHOD autoReadExcel.

    MODIFY ENTITIES OF zcjs_i_xl_user IN LOCAL MODE
      ENTITY XLHead EXECUTE uploadExcelData FROM VALUE #( ( %is_draft    = keys[ 1 ]-%is_draft
                                                            %key-EndUser = keys[ 1 ]-EndUser
                                                            %key-FileId  = keys[ 1 ]-FileId ) )   .
  ENDMETHOD.
