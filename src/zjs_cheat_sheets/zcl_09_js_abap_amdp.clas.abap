...
CLASS zcl_09_js_abap_amdp DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS flight_analysis FOR TABLE FUNCTION zcjs_abap_table_function.
...
CLASS ZCL_09_JS_ABAP_AMDP IMPLEMENTATION.
...
* In the example, joined data from two CDS view entities are returned.
  ...
  METHOD get_flights
         ...
         LANGUAGE SQLSCRIPT
         OPTIONS READ-ONLY
         USING zcl_09_js_abap_amdp=>get_flights_amdp.
* Another AMDP procedure is called from SQLScript
    CALL "ZCL_09_JS_ABAP_AMDP=>GET_FLIGHTS_AMDP"(
      carrid => :carrid,
      fli_tab => :fli_tab );
  ENDMETHOD.
...
*    DATA amdp_proc_res TYPE zcl_09_js_abap_amdp=>carr_tab.
*
...
*    NEW zcl_09_js_abap_amdp( )->select_carriers(
...
*    TRY.
*        zcl_09_js_abap_amdp=>get_flights( EXPORTING carrid = 'LH'
...
*    TRY.
*        NEW zcl_09_js_abap_amdp( )->select_get_carr_fli(
*          EXPORTING carrid = 'LH'
...
* Simple data selection
  ...
  METHOD select_get_carr_fli
         ...
         LANGUAGE SQLSCRIPT
         OPTIONS READ-ONLY
         USING zcl_09_js_abap_amdp=>get_carr_fli.
* AMDP procedure to call an AMDP table function as specified after USING
    carr_fli_tab = SELECT *
                     FROM "ZCL_09_JS_ABAP_AMDP=>GET_CARR_FLI"(
                        carrid => :carrid );
  ENDMETHOD.
ENDCLASS.
