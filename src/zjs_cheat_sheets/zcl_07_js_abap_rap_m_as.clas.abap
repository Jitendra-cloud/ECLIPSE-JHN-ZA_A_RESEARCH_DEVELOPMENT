...
"! </ul>
CLASS zcl_07_js_abap_rap_m_as DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS ZCL_07_JS_ABAP_RAP_M_AS IMPLEMENTATION.
  ...
  METHOD if_oo_adt_classrun~main.
    ...
    MODIFY ENTITY zdemo_abap_rap_ro_m_as
      CREATE AUTO FILL CID FIELDS ( num1 arithm_op num2 )
          WITH VALUE #( ( num1 = 1  arithm_op = '+' num2 = 2          )
                        ( num1 = 5  arithm_op = '-' num2 = 30         )
                        ( num1 = 3  arithm_op = '*' num2 = 3          )
                        ( num1 = 7  arithm_op = '/' num2 = 5          )
                        ( num1 = 2  arithm_op = 'P' num2 = 4          )
                        ( num1 = 10 arithm_op = 'P' num2 = 1000000000 )
                        ( num1 = 2  arithm_op = '/' num2 = 0          ) ) MAPPED DATA(m_cr)
                                                                          FAILED DATA(f_cr)
                                                                          REPORTED DATA(r_cr).
    COMMIT ENTITIES.
    ...
    SELECT id, num1, arithm_op, num2, calc_result, crea_date_time, lchg_date_time
     FROM zdemo_abap_tabca ORDER BY lchg_date_time INTO TABLE @DATA(dbtab_entries).
      ...
      MODIFY ENTITY zdemo_abap_rap_ro_m_as
        ...
        WITH VALUE #( FOR wa IN m_cr-root ( id   = wa-id
                                            num2 = cl_abap_random_int=>create( seed = cl_abap_random=>seed( )
                                                                               min  = 1
                                                                               max  = 10 )->get_next( ) ) )  FAILED DATA(f_upd)
                                                                                                              REPORTED DATA(r_upd).
...
**********************************************************************
    ...
    IF lines( m_cr-root ) > 3.
      ...
      "EML MODIFY statement
      SELECT id, num1, arithm_op, num2, calc_result, crea_date_time, lchg_date_time
       FROM zdemo_abap_tabca ORDER BY lchg_date_time INTO TABLE @dbtab_entries.
...
**********************************************************************
    ...
    SELECT calc_result, crea_date_time
     FROM zdemo_abap_draft ORDER BY crea_date_time INTO TABLE @DATA(evt_log_entries).
