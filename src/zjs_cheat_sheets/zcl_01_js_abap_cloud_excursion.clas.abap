...
CLASS zcl_01_js_abap_cloud_excursion DEFINITION PUBLIC FINAL CREATE PUBLIC .
  ...
  PRIVATE SECTION.
    ...
    CONSTANTS package TYPE sxco_package VALUE 'ZA_GITHUB_PACKAGES'.  "'ZABAP_CHEAT_SHEETS'.
    ...
    CONSTANTS package4gen TYPE sxco_package VALUE 'ZJS_CHEAT_SHEETS'.
    "ID of a modifiable transport request
    CONSTANTS tr_req_id TYPE sxco_transport VALUE 'TRLK900965 '.  " 'ID0A123456'.
    "Name of the data element to be created
    CONSTANTS gen_dtel TYPE sxco_ad_object_name VALUE 'ZEJS_ABAP_DTEL'.
    "Name of the domain to be created
    CONSTANTS gen_doma TYPE sxco_ad_object_name VALUE 'ZOJS_ABAP_STATUS'.
    "Name of the database table to be created
    CONSTANTS gen_tabl TYPE sxco_dbt_object_name VALUE 'ZDJS_ABAP_BOOK'.
    "To enable the generation, specify "abap_true" for the contant value.
    CONSTANTS generation_ok TYPE abap_bool VALUE abap_true. "abap_false.

ENDCLASS.



CLASS ZCL_01_JS_ABAP_CLOUD_EXCURSION IMPLEMENTATION.
...
******************************Using-String-To-JSON-Convertor***+***Write/Printing-TableData-W/o-Structure*****************************************************
    ...
    DATA(number_of_months) = cl_abap_random_int=>create(  seed = cl_abap_random=>seed( ) min = 5
                                                                                         max = 12 )->get_next( ).
...
**************************************************Getting-List-of-Released-Objects******************************************************************************
...
***********************************************Operations-On-Packages-(Existancy-&-Name)************************************************************************
...
*    ENDIF.
*********************************Searching-Classes-Interfaces-&-BDEF***************************************
...
*
*    DATA(package_handle_2) = xco_cp_abap_repository=>package->for( 'ZA_CDS_PRACTICE' ).           "Injected Extra Code By Me
*
*    DATA(dbtabs) = xco_cp_abap_repository=>objects->tabl->database_tables->where( VALUE #( ( filter5 )
*      ) )->in( package_handle_2 )->get( ).
*
...
*    DATA(db_table) = CONV sxco_dbt_object_name( 'ZDJS_DISCOUNT' ).   "( 'ZDEMO_ABAP_CARR' ).
...
*
*    IF b2_exists IS NOT INITIAL.
*      db_table = 'ZDJS_DISCOUNT'. "'ZDEMO_ABAP_CARR'.
*      DATA(db_field) = CONV sxco_ad_field_name( 'PARTNER' ).
...
****************************Information about a particular database table field
**
***********************************Getting Data Element-Related Information
...
***********************************Getting Data Element-Related Information
*
***********************************Getting Table Type-Related Information
*    out->write( heading( `6) Getting Table Type-Related Information` ) ).
*
*    DATA(tab_type) = CONV sxco_ad_object_name( 'STRING_TABLE' ).  "( 'STRING_TABLE' ).
...
***********************************Getting Table Type-Related Information
**
*********************************Getting CDS View Entity-Related Information
*    out->write( heading( `7) Getting CDS View Entity-Related Information` ) ).
*
*    DATA(cds) = CONV sxco_cds_object_name( 'ZCJS_I_DISCOUNT' ). "( 'ZDEMO_ABAP_CDS_VE_ASSOC' ).
...
*    out->write( |\nCDS View Entity Example with Parameters| ).
*                    DATA(cds) = CONV sxco_cds_object_name( 'ZCJS_I_DISCOUNT' ).
...
********************************Getting Interface-Related Information
*
...
********************************Getting Class-Related Information
...
********************************Getting AMDP-Related Information
...
********************************Getting Date and Time Information Using XCO
...
********************************Generating Repository Objects Using XCO**********************************************************************************
