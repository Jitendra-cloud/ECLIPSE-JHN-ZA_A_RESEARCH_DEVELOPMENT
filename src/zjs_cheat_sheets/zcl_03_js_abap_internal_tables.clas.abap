...
"! <strong>disclaimer</strong> in the ABAP Doc comment of class {@link zcl_01_js_abap_aux}.</p>
CLASS zcl_03_js_abap_internal_tables DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS ZCL_03_JS_ABAP_INTERNAL_TABLES IMPLEMENTATION.


  METHOD class_constructor.
    zcl_01_js_abap_aux=>fill_dbtabs( ).
    fill_dbtabs( ).
  ENDMETHOD.
  ...
  METHOD fill_dbtabs.
    ...
    DELETE FROM zjs_abap_tab1.
    DELETE FROM zjs_abap_tab2.

    MODIFY zjs_abap_tab1 FROM TABLE @( VALUE #(
    ...
    MODIFY zjs_abap_tab2 FROM TABLE @( VALUE #(
  ...
  METHOD if_oo_adt_classrun~main.
    ...
    ""---------------------------------------------------------
            ...
            DATA itab_sql TYPE TABLE OF zjs_abap_tab1 WITH NON-UNIQUE KEY client key_field.
    ""---------------------------------------------------------
...
*    SELECT * FROM zjs_abap_fli INTO TABLE @DATA(it_7).
*
*    "Instead of
*    DATA it_8 TYPE TABLE OF zjs_abap_fli WITH EMPTY KEY.
*    SELECT * FROM zjs_abap_fli INTO TABLE @it_8.
*
*    "Using FINAL
*    SELECT * FROM zjs_abap_fli INTO TABLE @FINAL(it_9).
*
*    out->write( zcl_01_js_abap_aux=>no_output ).
*******************************************Creating Internal Tables By Inline Declaration***********************************************************************

************************From**Populating internal tables by adding a line (structure) using APPEND ... TO/INSERT ... INTO********************************************
*
*    out->write( zcl_01_js_abap_aux=>heading( `2) Populating internal tables by adding a line (structure) using APPEND ... TO/INSERT ... INTO` ) ).
*
*    TYPES: BEGIN OF st_a,
*             num  TYPE i,
*             str  TYPE string,
...
***************************Populating internal tables by adding a line (structure) using APPEND ... TO/INSERT ... INTO********************************************
*
***********************************************************************
*
*    out->write( zcl_01_js_abap_aux=>heading( `3) Adding an initial line` ) ).
*
...
***********************************************************************
*
*    out->write( zcl_01_js_abap_aux=>heading( `4) Adding a line and assigning the added line to a field symbol or data reference variable` ) ).
*
...
***********************************************************************
*
*    out->write( zcl_01_js_abap_aux=>heading( `5) Adding all lines from another internal table (LINES OF addition)` ) ).
*
...
***********************************************************************
*
*    out->write( zcl_01_js_abap_aux=>heading( `6) Adding multiple lines from another internal table with a specified index range` ) ).
*
...
***********************************************************************
*
*    out->write( zcl_01_js_abap_aux=>heading( `7) Inserting one line or multiple lines from another internal table at a specific position (INDEX addition)` ) ).
*
...
***********************************************************************
*
*    out->write( zcl_01_js_abap_aux=>heading( `8) Adding lines using the VALUE operator` ) ).
*
...
**    it_d =  VALUE #(           ( num = 14 str =  `B` char = 'cc' )
...
*************************To***************************Adding lines using the VALUE operator using 'VALUE #'  &  'BASE'*******************************************

*****************************************************CORRESPONDING operator / MOVE-CORRESPONDING statements******************************************************
*    out->write( zcl_01_js_abap_aux=>heading( `9) CORRESPONDING operator / MOVE-CORRESPONDING statements` ) ).
*
...
*********************DEEP addition to the CORRESPONDING operator / EXPANDING NESTED TABLES addition to MOVE-CORRESPONDING statements*****************************
*    out->write( zcl_01_js_abap_aux=>heading( `10) DEEP addition to the CORRESPONDING operator / EXPANDING NESTED TABLES addition to MOVE-CORRESPONDING statements` ) ).
*
...
*    out->write( zcl_01_js_abap_aux=>heading( `11) CORRESPONDING with lookup table` ) ).
*
...
*
*    DATA(it_lk1) = VALUE it_type( ( character = 'a' ) ( character = 'b' ) ( character = 'c' ) ( character = 'd' ) ( character = 'e' ) ( character = 'f' ) ).
...
**      lookup_table TYPE SORTED TABLE OF s_lk3 WITH NON-UNIQUE KEY d,
**      lookup_tab_type TYPE HASHED TABLE OF s_lk1 WITH UNIQUE KEY character.
*      it_lk6       TYPE STANDARD TABLE OF s_lk4.
*
*    it_lk3 = VALUE #(  ( a = `1a`  b = `1b` c = `---` d = `---` e = `---` f = `---` )
*                       ( a = `2a`  b = `2b` c = `---` d = `---` e = `---` f = `---` )
*                       ( a = `3a`  b = `3b` c = `---` d = `---` e = `---` f = `---` ) ).
*
*    it_lk4 = it_lk3.
...
*
*    lookup_table = VALUE #(
*                       ( a = `4a` b = `4b` c = `1a` d = `1b` e = `5a` g = `5b` )
*                       ( a = `6a` b = `6b` c = `3a` d = `3b` e = `7a` g = `7b` ) ).
*
...
**    it_lk3 = CORRESPONDING #( it_lk3 FROM lookup_table USING d = b ).
**    it_lk1 = CORRESPONDING #( it_lk1 FROM lookup_tab USING character = character ).
*
...
********************************************************Creating anonymous internal tables with the NEW operator**************************************************
*    out->write( zcl_01_js_abap_aux=>heading( `12) Creating anonymous internal tables with the NEW operator` ) ).
*
*    TYPES: BEGIN OF s,
*             a TYPE c LENGTH 3,
*             b TYPE i,
...
*                                   ( a = 'bbb' b = 2 ) ).
*
*     "DATA(dref_tab2) = value tab_type( ( a = 'ccc' b = 3 )
*     "                                  ( a = 'ddd' b = 4 ) ).
*
*     "data dref_tab3 TYPE tab_type.
*     "data(dref_tab4) = dref_tab->*.
*     "dref_tab3 = CORRESPONDING #(  dref_tab4  ).
*
...
*************************************************************FILTER: Filtering internal table by condition********************************************************
*    out->write( zcl_01_js_abap_aux=>heading( `13) FILTER: Filtering internal table by condition` ) ).
*
...
*
*    "Self Testing with Standard Table.
*    "DATA fi_tab99 TYPE STANDARD TABLE OF fi_str.
*    "fi_tab99 = fi_tab1.
*    "data fi_tab999 TYPE SORTED TABLE OF fi_str WITH NON-UNIQUE KEY  b .
*    "fi_tab999 = fi_tab99.
*    "data(f999) = FILTER #( fi_tab999 WHERE b = 'aaa' ).
*
...
*
*    filter_tab1 = VALUE #( ( 3 ) ( 5 ) ( 7 ) ( 8 ) ( 9 ) ).
*    filter_tab2 = VALUE #( ( 3 ) ( 5 ) ( 9 ) ).
*
...
***************************************************************READ TABLE: Reading a single line by index*********************************************************
*    out->write( zcl_01_js_abap_aux=>heading( `14) READ TABLE: Reading a single line by index` ) ).
...
*
***********************************************************************
*    out->write( zcl_01_js_abap_aux=>heading( `15) System field setting with READ TABLE statements` ) ).
*
...
***************************************************************READ TABLE: Reading a single line by index*********************************************************
    ...
    out->write( zcl_01_js_abap_aux=>heading( `16) READ TABLE: Reading a single line using table keys` ) ).
*
...
*    DATA(non_key) = VALUE st_j( str = 'C' ).  "Injected by me for self test
*
*    READ TABLE it_k FROM pr_key INTO struc_i.
*    read TABLE it_k from non_key into struc_i.  "Injected by me for self test
...
*
**********************************************************************
    out->write( zcl_01_js_abap_aux=>heading( `17) READ TABLE: Reading a single line using a free key` ) ).
*
...
*
**********************************************************************
    out->write( zcl_01_js_abap_aux=>heading( `18) Examples for addressing individual components of read lines` ) ).
*
...
*
***********************************************************************
    out->write( zcl_01_js_abap_aux=>heading( `19) READ TABLE: COMPARING / TRANSPORTING additions` ) ).
*
*    "Comparing fields and specifying fields to be transported
*
*                    ""------------------Injected by me due commented above------------------------
*                                TYPES: BEGIN OF st_i,
*                                     num  TYPE i,
*                                     str  TYPE string,
*                                     char TYPE c LENGTH 2,
...
**            DATA struc_m2 TYPE st_i.
*    DATA it_n LIKE it_m.
**            DATA it_n2 TYPE ty_tab_h.
*    DATA it_o LIKE it_m.
...
*
*    out->write( zcl_01_js_abap_aux=>no_output ).
    out->write( zcl_01_js_abap_aux=>heading( `20) READ TABLE: CASTING / ELSE UNASSIGN additions` ) ).
*
...
*
*   out->write( zcl_01_js_abap_aux=>no_output ).
    out->write( zcl_01_js_abap_aux=>heading( `21) READ TABLE: Specifying a WHERE Condition` ) ).
*
*    "Creating and populating demo internal tables
...
*
*    out->write( zcl_01_js_abap_aux=>no_output ).
    out->write( zcl_01_js_abap_aux=>heading( `22) Table expressions: Reading table lines by index` ) ).
*
*     "Creating and populating demo internal tables
...
**********************************************************************
*
    out->write( zcl_01_js_abap_aux=>heading( `23) Table expressions: Reading table lines by table key` ) ).
*
*    "------------------ TABLE KEY addition ------------------
*
*    "Explicitly specifying the primary table key
...
***********************************************************************
*
    out->write( zcl_01_js_abap_aux=>heading( `24) Table expressions: Reading table lines using free keys` ) ).
*
...
***********************************************************************
*
    out->write( zcl_01_js_abap_aux=>heading( `25) Table expressions: Assigning table lines to a field symbol, data reference variables pointing to a table line` ) ).
*
...
***********************************************************************
*
    out->write( zcl_01_js_abap_aux=>heading( `26) Table expressions: Specifying table expressions as operands in constructor expressions with VALUE and REF` ) ).
*
*    line = VALUE #( itab[ 2 ] ).
...
***********************************************************************
*
    out->write( zcl_01_js_abap_aux=>heading( `27) Table expressions: Specifying a default value for lines that are not found to avoid an exception` ) ).
*
*    TRY.
*        line = itab[ 4 ].
...
*
*    line = VALUE #( itab[ 5 ] DEFAULT itab[ 2 ]  ).
*    line = VALUE #( itab[ 6 ] DEFAULT VALUE #( comp1 = 6 comp2 = 5 comp3 = 1 comp4 = 'DDD' ) ).
*
*    out->write( zcl_01_js_abap_aux=>no_output ).
*
***********************************************************************
*
    out->write( zcl_01_js_abap_aux=>heading( `28) Table expressions: Field symbols and dereferenced data references specified before the square brackets` ) ).
*
...
***********************************************************************
*
    out->write( zcl_01_js_abap_aux=>heading( `29) Table expressions: Reading individual components of table lines` ) ).
*
...
***********************************************************************
*
    out->write( zcl_01_js_abap_aux=>heading( `30) Table expressions: Chaining table expressions in the context of nested internal tables` ) ).
*
...
***********************************************************************
*
    out->write( zcl_01_js_abap_aux=>heading( `31) Table expressions in write positions: Writes on the entire line and writes on individual components` ) ).
*
...
***********************************************************************
*
    out->write( zcl_01_js_abap_aux=>heading( `32) Checking the Existence of a Line in an Internal Table` ) ).
*
*    "Read using a key
*            DATA itab_std2 TYPE TABLE OF s_demo WITH NON-UNIQUE KEY comp1 WITH NON-UNIQUE SORTED KEY sk COMPONENTS comp2 comp3.
*            itab_std2 = itab.
*
*    READ TABLE itab_std2 WITH KEY comp1 = 2 TRANSPORTING NO FIELDS.
*
*    IF sy-subrc = 0.
...
*    READ TABLE itab_std2 INDEX 1 TRANSPORTING NO FIELDS.
*
*    IF sy-subrc = 0.
*      ...
*    ENDIF.
*
*    "Read using the key
*    IF line_exists( itab_std2[ comp1 = 2 ] ).
*      ...
*    ENDIF.
*
*    "Read using the index
*    IF line_exists( itab_std2[ 2 ] ).
*      ...
*    ENDIF.
*
*    out->write( zcl_01_js_abap_aux=>no_output ).
...
***********************************************************************
*
    out->write( zcl_01_js_abap_aux=>heading( `33) Checking the index of a line in an internal table` ) ).
*
...
***********************************************************************
*
    out->write( zcl_01_js_abap_aux=>heading( `34) Checking How Many Lines Exist in an Internal Table` ) ).
*
*
...
***********************************************************************
*
    out->write( zcl_01_js_abap_aux=>heading( `35.2) Replace the LOOP for REDUCE operator` ) ).
*
*        TYPES:
*      BEGIN OF y_ekpo,
*        ebeln TYPE c LENGTH 10,
*        ebelp TYPE c LENGTH 2,
*        netwr TYPE p LENGTH 5 DECIMALS 2,
*      END OF y_ekpo,
...
*        kposn TYPE c LENGTH 2,
*        kschl TYPE c LENGTH 4,
*        kwert TYPE p LENGTH 5 DECIMALS 2,
*      END OF y_komv,
...
**    ------------------------------------------------*
*        types: netwr TYPE p LENGTH 5 DECIMALS 2.
*
*        LOOP AT it_ekpo
*          ASSIGNING FIELD-SYMBOL(<fs_ekpo>).
*          <fs_ekpo>-netwr = REDUCE netwr( INIT val TYPE netwr
*                                          FOR wa2 IN FILTER #( it_komv USING KEY key_kposn
*                                                                       WHERE kposn EQ CONV #( <fs_ekpo>-ebelp ) )
*                                          NEXT val = val + wa2-kwert ).
*        ENDLOOP.
*
***********************************************************************
*
    out->write( zcl_01_js_abap_aux=>heading( `35) Getting Table (Type) Information at Runtime` ) ).
*
*    TYPES tab_info_type TYPE SORTED TABLE OF zdjs_abap_flsch
...
*            """""""""""""""""""""Below Code Copied from 'Creating anynomous internal table' because commented """"""""""""""""
*                             TYPES: BEGIN OF s,
*                                     a TYPE c LENGTH 3,
*                                     b TYPE i,
*                                   END OF s,
...
***********************************************************************
*
    out->write( zcl_01_js_abap_aux=>heading( `Processing Multiple Internal Table Lines Sequentially` ) ).
*
...
**********************************************************************
*
    out->write( zcl_01_js_abap_aux=>heading( `37) Loop statements with different table key specifications` ) ).
*
...
***********************************************************************
*
    out->write( zcl_01_js_abap_aux=>heading( `38) Restricting the Area of a Table to Be Looped Over` ) ).
*
...
***********************************************************************
*
    out->write( zcl_01_js_abap_aux=>heading( `39) Defining the Step Size and the Direction of Loop Passes` ) ).
*
...
***********************************************************************
*
    out->write( zcl_01_js_abap_aux=>heading( `40) Interrupting and Exiting Loops` ) ).
*
...
***********************************************************************
*
    out->write( zcl_01_js_abap_aux=>heading( `41) Iteration Expressions` ) ).
*
...
***********************************************************************
*
    out->write( zcl_01_js_abap_aux=>heading( `41) Retrieving values of one column in ` &&
...
*    out->write( |\n| ).
*
    out->write( zcl_01_js_abap_aux=>heading( `42) Looping across 2 tables ` &&
         `and retrieving values based on conditions` ) ).
*
*    "Internal table type
*    DATA(it_int) = VALUE ty_int_tab( FOR x = 1 WHILE x <= 4 ( x ) ).
*
*    DATA(itab_for_2tab) = VALUE  ttype_loop( FOR ls3 IN it_int
...
***********************************************************************
*
    out->write( zcl_01_js_abap_aux=>heading( `43) Inserting and Deleting Lines in Internal Tables in Loops` ) ).
*
...
***********************************************************************
*
    out->write( zcl_01_js_abap_aux=>heading( `44) Selecting multiple rows from a database table into an internal table` ) ).
*
*    SELECT FROM zjs_abap_tab1
...
***********************************************************************
*
    out->write( zcl_01_js_abap_aux=>heading(  `45) Sequentially adding multiple rows from a database table to an internal table` ) ).
*
*
*    SELECT FROM zjs_abap_tab1
*      FIELDS *
...
***********************************************************************
*
    out->write( zcl_01_js_abap_aux=>heading( `47) Adding multiple rows from a database table ` &&
      `to an internal table that has a different line type than the ` &&
...
***********************************************************************
*
    out->write( zcl_01_js_abap_aux=>heading( `48) Adding multiple rows from an internal table ` &&
...
***********************************************************************
*
    out->write( zcl_01_js_abap_aux=>heading( `49) Combining data of multiple tables into an` &&
      ` internal table using an inner join` ) ).
*
*    "Filling table to be selected from
*    itab_sql =  VALUE #( ( key_field = 500 char1 = 'uuu' char2 = 'vvv' num1 = 501 num2  = 502 )
*                         ( key_field = 600 char1 = 'www' char2 = 'xxx' num1 = 601 num2  = 602 ) ).
*
...
***********************************************************************
*
    out->write( zcl_01_js_abap_aux=>heading( `50) Filling internal table ` &&
      `using a subquery (1)` ) ).
*
...
***********************************************************************
*
    out->write( zcl_01_js_abap_aux=>heading( `51) Filling internal table ` &&
      `using a subquery (2)` ) ).
*
...
*           WHERE key_field = zjs_abap_tab2~key_field )
...
***********************************************************************
*
    out->write( zcl_01_js_abap_aux=>heading( `52) Filling an internal table from a table ` &&
...
***********************************************************************
*
    out->write( zcl_01_js_abap_aux=>heading( `53) Adding content from a database to internal` &&
...
***********************************************************************
*
    out->write( zcl_01_js_abap_aux=>heading( `54) SELECT Queries with Internal Tables as Data Sources` ) ).
*
...
***********************************************************************
*
    out->write( zcl_01_js_abap_aux=>heading( `55) Excursion: Joining/Merging Internal Tables into Internal Tables` ) ).
...
**********************************************************************
    ...
    out->write( zcl_01_js_abap_aux=>heading( `56) Sorting internal tables` ) ).
...
**********************************************************************
    ...
    out->write( zcl_01_js_abap_aux=>heading( `57) Sorting by primary table key` ) ).
...
**********************************************************************
    ...
    out->write( zcl_01_js_abap_aux=>heading( `58) Sorting by primary table key in ascending` &&
      ` order` ) ).
...
**********************************************************************
    ...
    out->write( zcl_01_js_abap_aux=>heading( `59) Sorting by primary table key respecting all ` &&
      `non-numeric fields` ) ).
...
**********************************************************************
    ...
    out->write( zcl_01_js_abap_aux=>heading( `60) Sorting by primary table key in ` &&
      `descending order` ) ).
...
**********************************************************************
    ...
    out->write( zcl_01_js_abap_aux=>heading( `61) Sorting by explicitly specified component (1)` ) ).
...
**********************************************************************
    ...
    out->write( zcl_01_js_abap_aux=>heading( `62) Sorting by explicitly specified component (2)` ) ).
...
**********************************************************************
    ...
    out->write( zcl_01_js_abap_aux=>heading( `63) Sorting by multiple explicitly specified` &&
      ` components` ) ).
...
**********************************************************************
    ...
    out->write( zcl_01_js_abap_aux=>heading( `64) Sorting by respecting the values of all` &&
      ` components` ) ).
...
**********************************************************************
    ...
    out->write( zcl_01_js_abap_aux=>heading( `65) Modifying Internal Table Content: Direct modification of recently read table lines` ) ).
    ...
    DATA it_so TYPE SORTED TABLE OF ty_struc WITH UNIQUE KEY PRIMARY_KEY alias p00 COMPONENTS comp1
                                             WITH NON-UNIQUE SORTED KEY s00 COMPONENTS comp2.
    DATA it_ha TYPE HASHED TABLE OF ty_struc WITH UNIQUE KEY PRIMARY_KEY ALias P00 COMPONENTS comp1.
...
**********************************************************************
    ...
    out->write( zcl_01_js_abap_aux=>heading( `66) Modifying Internal Table Content: MODIFY statements` ) ).
...
**********************************************************************
    ...
    out->write( zcl_01_js_abap_aux=>heading( `67) Deleting internal table content` ) ).
...
**********************************************************************
    ...
    out->write( zcl_01_js_abap_aux=>heading( `68) Deleting adjacent duplicate entries` ) ).
...
**********************************************************************
    ...
    out->write( zcl_01_js_abap_aux=>heading( `69) Deleting adjacent duplicates based on` &&
      ` primary table key` ) ).
...
**********************************************************************
    ...
    out->write( zcl_01_js_abap_aux=>heading( `70) Deleting adjacent duplicates by comparing ` &&
      `all field values` ) ).
...
**********************************************************************
    ...
    out->write( zcl_01_js_abap_aux=>heading( `71) Deleting adjacent duplicates by comparing ` &&
...
**********************************************************************
    ...
    out->write( zcl_01_js_abap_aux=>heading( `72) Deleting adjacent duplicates by using a` &&
      ` table key` ) ).
...
**********************************************************************
    ...
    out->write( zcl_01_js_abap_aux=>heading( `73) Deleting the entire internal table content` ) ).
...
**********************************************************************
    ...
    out->write( zcl_01_js_abap_aux=>heading( `74) Grouping Internal Tables` ) ).
...
**********************************************************************
    ...
    out->write( zcl_01_js_abap_aux=>heading( `75) Collecting Values` ) ).
...
**********************************************************************
    ...
    out->write( zcl_01_js_abap_aux=>heading( `Excursions` ) ).
...
**********************************************************************
    ...
    out->write( zcl_01_js_abap_aux=>heading( `77) Empty keys in internal table created inline` ) ).
...
**********************************************************************
    ...
    out->write( zcl_01_js_abap_aux=>heading( `78) Ranges tables` ) ).
...
**********************************************************************
    ...
    out->write( zcl_01_js_abap_aux=>heading( `79) Creating Internal Tables Dynamically` ) ).
